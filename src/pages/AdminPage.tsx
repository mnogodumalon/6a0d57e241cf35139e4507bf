...
const AENDERUNGSVORSCHLAG_FIELDS = [
  { key: 'titel', label: 'Titel des Vorschlags', type: 'string/text' },
  { key: 'kategorie', label: 'Kategorie', type: 'lookup/select', options: [{ key: 'benutzeroberflaeche', label: 'Benutzeroberfläche' }, { key: 'funktionalitaet', label: 'Funktionalität' }, { key: 'performance', label: 'Performance' }, { key: 'sicherheit', label: 'Sicherheit' }, { key: 'barrierefreiheit', label: 'Barrierefreiheit' }, { key: 'sonstiges', label: 'Sonstiges' }] },
  { key: 'betroffener_bereich', label: 'Betroffener Bereich', type: 'string/text' },
  { key: 'beschreibung', label: 'Beschreibung des Vorschlags', type: 'string/textarea' },
  { key: 'screenshots', label: 'Screenshots', type: 'file' },
  { key: 'einreicher_vorname', label: 'Vorname', type: 'string/text' },
  { key: 'einreicher_nachname', label: 'Nachname', type: 'string/text' },
  { key: 'einreicher_email', label: 'E-Mail-Adresse', type: 'string/email' },
  { key: 'einreichungsdatum', label: 'Datum der Einreichung', type: 'date/date' },
  ...
  { key: 'status', label: 'Status', type: 'lookup/select', options: [{ key: 'offen', label: 'Offen' }, { key: 'in_pruefung', label: 'In Prüfung' }, { key: 'in_bearbeitung', label: 'In Bearbeitung' }, { key: 'umgesetzt', label: 'Umgesetzt' }, { key: 'abgelehnt', label: 'Abgelehnt' }, { key: 'zurueckgestellt', label: 'Zurückgestellt' }] },
  ...
  { key: 'umsetzungskommentar', label: 'Kommentar zur Umsetzung', type: 'string/textarea' },
  { key: 'erledigungsdatum', label: 'Erledigungsdatum', type: 'date/date' },
];

const ENTITY_TABS = [
  { key: 'aenderungsvorschlag', label: 'Änderungsvorschlag', pascal: 'Aenderungsvorschlag' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('aenderungsvorschlag');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'aenderungsvorschlag': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'aenderungsvorschlag': return (data as any).aenderungsvorschlag as Aenderungsvorschlag[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'aenderungsvorschlag': return AENDERUNGSVORSCHLAG_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'aenderungsvorschlag': return {
        create: (fields: any) => LivingAppsService.createAenderungsvorschlagEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateAenderungsvorschlagEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteAenderungsvorschlagEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'aenderungsvorschlag' || dialogState?.entity === 'aenderungsvorschlag') && (
        <AenderungsvorschlagDialog
          open={createEntity === 'aenderungsvorschlag' || dialogState?.entity === 'aenderungsvorschlag'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'aenderungsvorschlag' ? handleUpdate : (fields: any) => handleCreate('aenderungsvorschlag', fields)}
          defaultValues={dialogState?.entity === 'aenderungsvorschlag' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Aenderungsvorschlag']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Aenderungsvorschlag']}
        />
      )}
      {viewState?.entity === 'aenderungsvorschlag' && (
        <AenderungsvorschlagViewDialog
          open={viewState?.entity === 'aenderungsvorschlag'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'aenderungsvorschlag', record: r }); }}
        />
      )}
