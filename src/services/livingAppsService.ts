...
import type { Aenderungsvorschlag, CreateAenderungsvorschlag } from '@/types/app';
...
export class LivingAppsService {
  // --- AENDERUNGSVORSCHLAG ---
  static async getAenderungsvorschlag(): Promise<Aenderungsvorschlag[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.AENDERUNGSVORSCHLAG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Aenderungsvorschlag[];
    return enrichLookupFields(records, 'aenderungsvorschlag');
  }
  static async getAenderungsvorschlagEntry(id: string): Promise<Aenderungsvorschlag | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.AENDERUNGSVORSCHLAG}/records/${id}`);
    const record = { record_id: data.id, ...data } as Aenderungsvorschlag;
    return enrichLookupFields([record], 'aenderungsvorschlag')[0];
  }
  static async createAenderungsvorschlagEntry(fields: CreateAenderungsvorschlag) {
    return callApi('POST', `/apps/${APP_IDS.AENDERUNGSVORSCHLAG}/records`, { fields: cleanFieldsForApi(fields as any, 'aenderungsvorschlag') });
  }
  static async updateAenderungsvorschlagEntry(id: string, fields: Partial<CreateAenderungsvorschlag>) {
    return callApi('PATCH', `/apps/${APP_IDS.AENDERUNGSVORSCHLAG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'aenderungsvorschlag') });
  }
  static async deleteAenderungsvorschlagEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.AENDERUNGSVORSCHLAG}/records/${id}`);
  }
